...
func (u *cuser) DealWithRequests(db *sql.DB) {
	...
	isHTTPLink := strings.Contains(avatar_link, ".")
	if isHTTPLink { // 用户存在自定义头像外链，发送用户 id 及后缀
...
func (u *cuser) fork(db *sql.DB, command string) {
	...
	if record.private || record.private && record.linkpass != args[2] ||
...
func (u *cuser) touch(db *sql.DB, command string) {
	// 指令格式：TOUCH<SEP>创建文件名<SEP>创建路径<SEP>资源类型（文件/目录）
	var valid bool = true
